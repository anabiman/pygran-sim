        ...
        if "mesh" in self.params:
            for mesh in self.params["mesh"]:
                ...
                if "file" in self.params["mesh"][mesh]:
                    self.params["species"] += (
                        {"material": self.params["mesh"][mesh]["material"]},
                    )
                    self.params["nSS"] += 1

                    # Get absolute path filename
                    self.params["mesh"][mesh]["file"] = os.path.abspath(
                        self.params["mesh"][mesh]["file"]
                    )
                        ...
                        if ss["material"][item][1] == "peratomtype":
                            ...
                            for nss in range(self.params["nSS"]):
                                ...
                                prop = np.sqrt(
                                    ...
                                    * float(
                                        self.params["species"][nss]["material"][item][2]
                                    )
                                )
                                self.materials[item] = self.materials[item] + (
                                    ("{}").format(prop),
                ...
                if "model" in self.params:
                    print(
                        "Model {} does not yet support estimation of contact period. Using a default value of {}".format(
                            self.params["model"], self.params["dt"]
                        )
                    )
