...
A C++ header template file for contact models in LIGGGHTS
...
#ifndef NORMAL_MODEL_{name}_H_
#define NORMAL_MODEL_{name}_H_
...
{
  template<>
  class NormalModel<{name}> : public NormalModelBase
  {
  public:
    ...
    inline void connectToProperties(PropertyRegistry & registry) {
      ...
      registry.connect("Yeff", Yeff,"model {name_lower}");
      registry.connect("Geff", Geff,"model {name_lower}");
      registry.connect("charVel", charVel,"model {name_lower}");
      ...
      if(viscous) {
        ...
        registry.connect("coeffMu", coeffMu,"model {name_lower} viscous");
        registry.connect("coeffStc", coeffStc,"model {name_lower} viscous");
        registry.connect("coeffRestMax", coeffRestMax,"model {name_lower} viscous");
        ...
        registry.connect("coeffRestLog", coeffRestLog,"model name_lower viscous");
      }

      // error checks on coarsegraining
      if(force->cg_active())
        error->cg(FLERR,"model name_lower");
